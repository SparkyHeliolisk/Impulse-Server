...
function isLvLBattle(p1, p2, id, status, types, score) {
	...
	if (Rooms.global.LvL[leagueId] && Rooms.global.LvL[leagueId].challenger && Rooms.global.LvL[leagueId].challenger === targetLeagueid || Rooms.global.LvL[leagueId] && Rooms.global.LvL[leagueId].challenging && Rooms.global.LvL[leagueId].challenging === targetLeagueid) {
		...
		if (status === 'start') {
			...
			lvlDisplay(room);
			room.add('|raw|<a href="/' + id + '">The League vs League battle between ' + WL.nameColor(p1, true) + ' (' + Chat.escapeHTML(leagues[leagueId].name) + ') and ' +
			...
			lvlDisplay(room);
			room.add("|raw|The League vs League battle between " + WL.nameColor(p1, true) + " and " + WL.nameColor(p2, true) + " ended with a tie. They have to have a rematch.");
			room.update();
		}
	}
}
...
exports.commands = {
	...
	regions: {
		...
		invite: function (target, room, user) {
			...
			leaguePM(WL.nameColor(user.name, true) + " has invited " + WL.nameColor(targetUser.name, true) + " to join the region.", leagueid);
		...
		accept: function (target, room, user) {
			...
			leaguePM(WL.nameColor(user.name, true) + " has accepted their invitation to join the region.", leagueid);
		...
		decline: function (target, room, user) {
			...
			leaguePM(WL.ameColor(user.name, true) + " has declined their invitation to join the region.", leagueid);
		...
		kick: function (target, room, user) {
			...
			if (Users(target) && Users(target).connected) Users(target).send("|popup||html|" + WL.nameColor(user.name, true) + " has kicked you from the region " + Chat.escapeHTML(leagues[leagueid].name) + ".");
		...
		leave: function (target, room, user) {
			...
			leaguePM(WL.nameColor(user.name, true) + " has left the region.", leagueid);
		...
		pm: function (target, room, user) {
			...
			leaguePM("Region announcement from " + WL.nameColor(user.name, true) + ":<br />" + Chat.escapeHTML(target), leagueid);
		},
		...
		rank: {
			...
			give: function (target, room, user) {
				...
				targetUser.send("|popup||html|" + WL.nameColor(user.name, true) + " has set your region rank in " + Chat.escapeHTML(leagues[leagueid].name) + " to " +
			...
			take: function (target, room, user) {
				...
				if (Users(targetUser) && Users(targetUser).connected) {
					Users(targetUser).send("|popup||html|" + WL.nameColor(user.name, true) + " has removed you from the region rank " + Chat.escapeHTML(rank) + " in " +
		...
		challenge: function (target, room, user, connection) {
			...
			user.prepBattle(Dex.getFormat('ubers').id, 'challenge', connection).then(result => {
				if (result) {
					...
					targetUser.send('|pm|' + user.getIdentity() + '|~|/raw <div class="infobox">' + WL.nameColor(user.name, true) + ' is challenging your league.</div>');
					user.send('|pm|' + targetUser.getIdentity() + '|~|/raw <div class="infobox">You are challenging ' + WL.nameColor(targetUser.userid, true) + '\'s league.');
		...
		/*rvs: {
			accept: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + ' has accepted the Region vs Region challenge against ' + Chat.escapeHTML(leagues[targetLeagueid].name), leagueId);
				leaguePM(WL.nameColor(user.name, true) + ' (' + leagues[leagueId].name + ') has accepted the Region vs Region challenge against your Region.', targetLeagueid);
			...
			deny: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + ' has declined the Region vs Region challenge against ' + Chat.escapeHTML(leagues[targetLeagueid].name), leagueId);
				leaguePM(WL.nameColor(user.name, true) + ' (' + leagues[leagueId].name + ') has declined the Region vs Region challenge against your region.', targetLeagueid);
				...
				leaguePM(WL.nameColor(user.name, true) + ' has forcibly ended the Region vs Region with ' + Chat.escapeHTML(leagues[targetLeagueid].name) + '.', leagueId);
			...
			invite: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has invited " + WL.nameColor(targetUser.name, true) + " to join the Region vs Region against " + Chat.escapeHTML(leagues[targetLeague.id].name), leagueId);
				targetUser.send("|popup||modal||html|" + WL.nameColor(user.name, true) + " has invited you to join the Region vs Region against " + Chat.escapeHTML(leagues[targetLeague.id].name) +
			...
			challenge: function (target, room, user) {
				...
				leaguePM(
					WL.nameColor(user.name, true) + ' (' + Chat.escapeHTML(getLeague(user.userid)) + ') has challenged your region to a Region vs Region (' +
					size + 'v' + size + ') in' +
				...
				leaguePM(
					WL.nameColor(user.name, true) + ' has challenged ' + Chat.escapeHTML(leagues[targetLeagueid].name) + ' to a Region vs Region (' +
		...
		points: {
			give: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has given your region " + amount + (amount === 1 ? " point." : " points."), league);
			},
			...
			take: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has taken " + amount + (amount === 1 ? " point " : " points ") + " from your region.", league);
			},
			...
			reset: function (target, room, user) {
				...
				WL.messageSeniorStaff("/html " + WL.nameColor(user.name, true) + " has reset all region points.");
				Rooms('staff').add("|raw|" + WL.nameColor(user.name, true) + " has reset all region points.").update();
		...
		help: function (target, room, user) {
			...
			return this.sendReply(
				...
				"/region icon [iconlink] - Sets a icon for your region, visible on /region list.<br />" +
